...
@RequestMapping(value = "stock/info", produces = APPLICATION_JSON)
public class StockInfoRest {
     ...
     */
    @GetMapping("{codes}")
    public Result<Void> overview(@PathVariable String codes) {
        Result<Void> result = Result.success();
        String[] codeArray = codes.split(COMMA);
        for (String code : codeArray) {
            result = stockInfoService.getStockInfo(code);
            if (!result.isSuccess()) {
                return result;
            }
